...
же размера. В результате растровые изображения выглядят пикселизированными.
...
Например, если предполагалось показать стрелочку рядом со словом «следующая»,
...
символьных шрифтов, и решили конкретно эту с помощью [области пользовательских символов Юникод][2].
Символы в этой области не связаны по смыслу ни c одной из букв в алфавите. Если
шрифт не доступен или не загрузился, то вместо бессмысленной буквы вы увидите
пустое место.

Но подождите, мы можем сделать ещё лучше!
...
Браузеры, не поддерживающие подключение сторонних шрифтов, и поисковые боты
проиндексируют сырой текст без лигатур, но все остальные, с поддержкой
возможности рендеринга подключаемых шрифтов и лигатур, получат приятную
векторную иконку. Это наилучшее решение, работающее на куда более высоком уровне
абстракции, чем HTML и CSS.
...
доступность и векторную графику. Напечатав «12:00», вы вызовете лигатуру, которая
...
векторные часы. Вообще-то, такие шрифты уже существуют; на странице
[http://timepiece.inostudio.de][5] вы найдёте несколько примеров.
...
объединить в один крохотный файл шрифта, который можно кэшировать и отображать в
...
Более того, чтобы помочь нам в создании шрифтов, было создано множество
...
Для создания шрифтов существует множество приложений. Некоторые из них
платные, предназначенные для профессионалов, но чаще всего используется
...
отличаться при разных размерах, обычно отличия заключаются в переменной толщине
...
Если вы хотите, чтобы дизайн иконок отличался для разных размеров, то вам
...
Первой попыткой в этой области является шрифт [Apple Color Emoji][8], который
...
использовать символьные шрифты и совершенствовать инструменты их разработки,
