...
кэшированием и количеством файлов и начали использовать CSS спрайты, собирая
...
графического спрайта. Этот способ, как и спрайты, дает выиграш за счет
...
петроглифов и декоративных элементов, несколько высоконагруженных сайтов
...
скорость и опыт взаимодействия пользователя, отказавшись от множества
крохотных файлов иконок в пользу созданного ими шрифта [Octicons][1].


Использование символьных шрифтов позволяет заменить растровые, подверженные
пикселизации изображения, векторными. Это важно, потому что растровые
изображения предназначены для использования на определённом разрешении экрана
и при повышеной плотности пикселей, как на современных экранах высокого
разрешения, их приходится масштабировать, что бы визуально они оставались того
...
повышенной четкости у нас появилась возможность еще больше оптимизировать
производительность и улучшить пользовательский опыт взаимодействия с сайтами.


## Доступность ##

Что касается доступности, то символьные шрифты ведут себя скорее как текст, а
не картинка.
...
[пользовательских символов Unicode][1]. Символы в этой области не связаны
смыслом ни c одной из букв в алфавите. Если шрифт не доступен или не загрузился,
то вместо бессмысленной буквы вы увидите пустое место.
...
котором буквы плвыно соединяются. Существует стандартные лигатуры, например:
...
Использование лигатур для замены последовательностей символов есть множество
...
Но все эти нефлэшевые решения используют растровые изображения, которые на
...
Более того, что бы помочь нам в создании шрифтов было изобретено моножество
...
пикселей останется таким же и при масштабировании его и до 120 пикселей, и до 6.
...
пользователь увидит ваш ваш сайт. Например, увеличение кегля шрифта не
приведет к автоматической смене иконки. Для большинства из нас это не проблема,
так как в основном символьные шрифты используются для иконок социальных сетей,
которые и так неплохо масштабируются.
...
уже есть в OSX Lion. Недостатками введения нового формата шрифтов является
низкая поддержка браузерами и инструментами разработки, так что пройдет еще
много времени пока его можно будет использовать.
...
вашего сайта, но скажется на его производительности. Наступило время начать
использовать символьные шрифты и улучшать инструменты их разработки, связанные
